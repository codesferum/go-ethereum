...
import (
	...
	"github.com/codesferum/go-sferum/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.sferum", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-sferum issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/sferum/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.sferum", "github.com/codesferum/go-sferum/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "sferum", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
