...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/sferum                                                            && \
	(cd $GOPATH/src/github.com/sferum && git clone --depth=1 https://github.com/codesferum/go-sferum) && \
  go build -v github.com/codesferum/go-sferum/cmd/faucet                                              && \
