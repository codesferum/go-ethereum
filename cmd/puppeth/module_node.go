...
var nodeDockerfile = `
FROM sferum/client-go:alpine-develop
...
RUN \
  echo '/geth init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.sferum/keystore/ && cp /signer.json /root/.sferum/keystore/' >> geth.sh && \{{end}}
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.sferum"],
