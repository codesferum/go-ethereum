...
import (
	...
	"github.com/codesferum/go-sferum/console"
	"github.com/codesferum/go-sferum/eth"
	"github.com/codesferum/go-sferum/ethclient"
	"github.com/codesferum/go-sferum/internal/debug"
...
var (
	...
	app = utils.NewApp(gitCommit, "the go-sferum command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2017 The go-sferum Authors"
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) {
		...
		if err := stack.Service(&sferum); err != nil {
			utils.Fatalf("sferum service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := sferum.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		// Set the gas price to the limits from the CLI and start mining
		sferum.TxPool().SetGasPrice(utils.GlobalBig(ctx, utils.GasPriceFlag.Name))
		if err := sferum.StartMining(true); err != nil {
